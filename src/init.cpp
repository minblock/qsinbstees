 ...
 * between qsinbsteesd, and qsinbstees-qt and non-server tools.
*/
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://qsinbsteesovate.io>";
...
void ThreadCheckInfinityNode(CConnman& connman)
{
    ...
    RenameThread("qsinbsteesovate-ps");
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if qsinbstees is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if qsinbstees is started while in a temporary directory.\n",
