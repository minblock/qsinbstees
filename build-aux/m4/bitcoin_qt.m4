...
dnl Output: If qt version is auto, set qsinbstees_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$qsinbstees_qt_want_version" = xauto && test "x$qsinbstees_qt_force" != xyes; then
    if test "x$qsinbstees_enable_qt" != xno; then
      AC_MSG_WARN([$1; qsinbstees-qt frontend will not be built])
    fi
    qsinbstees_enable_qt=no
    qsinbstees_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$qsinbstees_enable_qt" != xno && test "x$qsinbstees_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build qsinbstees-qt GUI (default=auto)])],
    [
     qsinbstees_qt_want_version=$withval
     if test "x$qsinbstees_qt_want_version" = xyes; then
       qsinbstees_qt_force=yes
       qsinbstees_qt_want_version=auto
     fi
    ],
    [qsinbstees_qt_want_version=auto])
...
dnl Outputs: qsinbstees_enable_qt, qsinbstees_enable_qt_dbus, qsinbstees_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$qsinbstees_cv_static_qt" = xyes; then
    _BITCOIN_QT_FIND_STATIC_PLUGINS
    AC_DEFINE(QT_STATICPLUGIN, 1, [Define this symbol if qt plugins are static])
    AC_CACHE_CHECK(for Qt < 5.4, qsinbstees_cv_need_acc_widget,[
      ...
      [qsinbstees_cv_need_acc_widget=yes],
      [qsinbstees_cv_need_acc_widget=no])
    ])
    if test "x$qsinbstees_cv_need_acc_widget" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    qsinbstees_enable_qt=yes
    qsinbstees_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      qsinbstees_enable_qt_test=no
    fi
    qsinbstees_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      qsinbstees_enable_qt_dbus=yes
    fi
  ...
  AC_MSG_RESULT([$qsinbstees_enable_qt (Qt5)])
...
dnl Output: qsinbstees_cv_qt5=yes|no
AC_DEFUN([_BITCOIN_QT_CHECK_QT5],[
  AC_CACHE_CHECK(for Qt 5, qsinbstees_cv_qt5,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
    ...
    [qsinbstees_cv_qt5=yes],
    [qsinbstees_cv_qt5=no])
])])
...
AC_DEFUN([_BITCOIN_QT_CHECK_QT58],[
  AC_CACHE_CHECK(for > Qt 5.7, qsinbstees_cv_qt58,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
    ...
    [qsinbstees_cv_qt58=yes],
    [qsinbstees_cv_qt58=no])
])])
...
dnl Output: qsinbstees_cv_static_qt=yes|no
dnl Output: Defines QT_STATICPLUGIN if plugins are static.
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, qsinbstees_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
      ...
      [qsinbstees_cv_static_qt=yes],
      [qsinbstees_cv_static_qt=no])
    ])
  if test "x$qsinbstees_cv_static_qt" = xyes; then
...
AC_DEFUN([_BITCOIN_QT_FIND_STATIC_PLUGINS],[
    if test "x$qt_plugin_path" != x; then
     ...
     m4_ifdef([PKG_CHECK_MODULES],[
       if test x$qsinbstees_cv_qt58 = xno; then
       ...
       if test "x$TARGET_OS" = xwindows; then
         AC_CACHE_CHECK(for Qt >= 5.6, qsinbstees_cv_need_platformsupport,[
           AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
               #include <QtCore/qconfig.h>
               #ifndef QT_VERSION
               #  include <QtCore/qglobal.h>
           ...
           [qsinbstees_cv_need_platformsupport=yes],
           [qsinbstees_cv_need_platformsupport=no])
         ])
         if test "x$qsinbstees_cv_need_platformsupport" = xyes; then
           if test x$qsinbstees_cv_qt58 = xno; then
...
dnl Inputs: qsinbstees_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If qsinbstees_qt_want_version is "auto", check for this version
dnl         first.
...
dnl Inputs: qsinbstees_qt_want_version (from --with-gui=). The version to use.
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  BITCOIN_QT_CHECK([
    if test "x$qsinbstees_qt_want_version" = xauto; then
