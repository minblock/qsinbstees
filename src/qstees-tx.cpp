...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " qsinbstees-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  qsinbstees-tx [options] <hex-tx> [commands]  Update hex-encoded qsinbstees transaction\n" +
            "or:     qsinbstees-tx [options] -create [commands]   Create hex-encoded qsinbstees transaction\n" +
            "\n";
...
static const struct {
    ...
    {"ALL", SIGHASH_ALL},
    {"NONE", SIGHASH_NONE},
    {"QSTEESGLE", SIGHASH_QSTEESGLE},
    {"ALL|ANYONECANPAY", SIGHASH_ALL|SIGHASH_ANYONECANPAY},
    {"NONE|ANYONECANPAY", SIGHASH_NONE|SIGHASH_ANYONECANPAY},
    {"QSTEESGLE|ANYONECANPAY", SIGHASH_QSTEESGLE|SIGHASH_ANYONECANPAY},
};
...
static void MutateTxSign(CMutableTransaction& tx, const std::string& flagStr)
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        if (!fHashQsteesgle || (i < mergedTx.vout.size()))
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded qsinbstees transaction
