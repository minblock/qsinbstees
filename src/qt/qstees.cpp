...
{
    return QCoreApplication::translate("qsinbstees-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. qsinbstees_de.qm (shortcut "de" needs to be defined in qsinbstees.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. qsinbstees_de_DE.qm (shortcut "de_DE" needs to be defined in qsinbstees.qrc)
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // qsinbstees: URIs or payment requests:
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(qsinbstees);
    Q_INIT_RESOURCE(qsinbstees_locale);
    ...
    /// 6. Determine availability of data and blocks directory and parse qsinbstees.conf
    ...
    if(!masternodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr("QSTEES Core"),
    ...
    // qsinbstees: links repeatedly have their payment requests routed to this process:
