...
class CScript(bytes):
    ...
    def __coerce_instance(cls, other):
        ...
        if iqsinbsteesstance(other, CScriptOp):
            other = bytes([other])
        elif iqsinbsteesstance(other, CScriptNum):
            if (other.value == 0):
                other = bytes([CScriptOp(OP_0)])
            else:
                other = CScriptNum.encode(other)
        elif iqsinbsteesstance(other, int):
            if 0 <= other <= 16:
        ...
        elif iqsinbsteesstance(other, (bytes, bytearray)):
    ...
    def __new__(cls, value=b''):
        if iqsinbsteesstance(value, bytes) or iqsinbsteesstance(value, bytearray):
