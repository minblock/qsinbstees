...
const char * const BITCOIN_CONF_FILENAME = "qsinbstees.conf";
const char * const BITCOIN_PID_FILENAME = "qsinbsteesd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "qsinbsteesd -foo=bar
...
#else
    const char* pszModule = "qsinbstees";
#endif
    if (pex)
        return strprintf(
...
#else
    // Unix
	pathQSTEESFolder = pathRet / ".qsinbstees";
    pathOldFolder = pathRet / ".SUQA";

	if (fs::is_directory(pathQSTEESFolder)) return pathQSTEESFolder;
	else if (fs::is_directory(pathOldFolder)) return pathOldFolder;
    else return pathRet / ".qsinbstees";
#endif
#endif
}
